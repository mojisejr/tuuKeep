...
/**
 * @title TuuCoinBaseSimple
 * @dev Simplified TuuCoinBase without ValidationLib for KUB testnet deployment
 * Handles standard token functionality, minting, and basic operations
 */
contract TuuCoinBaseSimple is ERC20, ERC20Burnable, AccessControl, Pausable {
     ...
     * Simplified version without ValidationLib checks for KUB testnet compatibility
     */
    constructor(
        address _accessControl,
        address _initialAdmin
    ) ERC20("TuuCoin", "TUU") {
        require(_accessControl != address(0), "TuuCoinBaseSimple: access control cannot be zero address");
        require(_initialAdmin != address(0), "TuuCoinBaseSimple: initial admin cannot be zero address");
    ...
    function mint(address to, uint256 amount) external onlyRole(MINTER_ROLE) {
        require(to != address(0), "TuuCoinBaseSimple: cannot mint to zero address");
        require(amount > 0, "TuuCoinBaseSimple: amount must be greater than zero");
        require(!mintingPaused, "TuuCoinBaseSimple: minting is paused");
        require(totalMinted + amount <= getEffectiveMaxSupply(), "TuuCoinBaseSimple: would exceed max supply");
    ...
    function batchMint(
        ...
        require(recipients.length == amounts.length, "TuuCoinBaseSimple: arrays length mismatch");
        require(recipients.length > 0, "TuuCoinBaseSimple: empty arrays");
        require(!mintingPaused, "TuuCoinBaseSimple: minting is paused");

        uint256 totalAmount = 0;
        for (uint256 i = 0; i < amounts.length; i++) {
            require(recipients[i] != address(0), "TuuCoinBaseSimple: cannot mint to zero address");
            require(amounts[i] > 0, "TuuCoinBaseSimple: amount must be greater than zero");
            totalAmount += amounts[i];
        }

        require(totalMinted + totalAmount <= getEffectiveMaxSupply(), "TuuCoinBaseSimple: would exceed max supply");
    ...
    function burn(uint256 amount) public override {
        require(!burningPaused, "TuuCoinBaseSimple: burning is paused");
        totalBurned += amount;
        super.burn(amount);
    }
    ...
    function _update(address from, address to, uint256 value) internal override {
        require(!transfersPaused || from == address(0) || to == address(0), "TuuCoinBaseSimple: transfers are paused");
    ...
    function adjustMaxSupply(uint256 newMaxSupply) external onlyRole(PLATFORM_ADMIN_ROLE) {
        require(isDynamicSupplyEnabled, "TuuCoinBaseSimple: dynamic supply not enabled");
        require(newMaxSupply >= totalMinted, "TuuCoinBaseSimple: new max supply below current minted");
        require(newMaxSupply <= MAX_SUPPLY * 2, "TuuCoinBaseSimple: cannot exceed 2x original max supply");
    ...
    {
        require(_newAccessControl != address(0), "TuuCoinBaseSimple: invalid access control address");
        require(_newAccessControl != address(accessControl), "TuuCoinBaseSimple: same access control address");
