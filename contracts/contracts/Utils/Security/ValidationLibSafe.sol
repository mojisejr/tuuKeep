...
/**
 * @title ValidationLibSafe
 * @dev Constructor-safe validation utilities for TuuKeep platform
 *
 * Key Features:
 * - Constructor-safe contract validation (doesn't fail during deployment)
 * - Maintains all security validation features from original ValidationLib
 * - Conditional validation logic for constructor vs runtime context
 * - Backward compatible with existing ValidationLib interface
 *
 * Usage:
 * - Use validateContractSafe() in constructors instead of validateContract()
 * - Use validateContract() in regular functions for strict validation
 * - Particularly important for contracts that reference other deployed contracts
 */
library ValidationLibSafe {
    ...
    /**
     * @dev CONSTRUCTOR-SAFE contract validation
     * @param addr The address to validate
     * @param context Description of what the contract represents
     *
     * This function is safe to use in constructors because it only validates
     * the address is not zero. Contract code existence is not checked during
     * constructor execution to avoid timing issues with deployment.
     */
    function validateContractSafe(address addr, string memory context) internal pure {
        validateAddress(addr, context);
        // NOTE: We skip _isContract() check in constructor-safe version
        // Contract code verification should be done post-deployment if needed
    }

    /**
     * @dev RUNTIME contract validation (strict validation)
     * @param addr The address to validate
     * @param context Description of what the contract represents
     *
     * This function performs full validation including contract code existence.
     * Use this in regular functions, NOT in constructors.
     */
     ...
     * @param addr The address to verify as contract
     * @param context Description of what the contract represents
     *
     * Use this function after deployment to verify that addresses
     * that were validated with validateContractSafe() actually
     * contain contract code.
     */
    function verifyContractPostDeployment(address addr, string memory context) internal view {
        validateContract(addr, context);
    }
}